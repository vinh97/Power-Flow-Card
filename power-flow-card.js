class PowerFlowCard extends HTMLElement {
  ...
  /**
   * Thuật toán tự động đo số lượng dòng hiển thị và căn đều khối chữ vào chính giữa tâm Y của Icon
   * @param {Array<HTMLElement>} elements - Danh sách các phần tử text dòng dữ liệu
   * @param {number} centerY - Tọa độ Y tâm của Icon tương ứng
   * @param {number} lineHeight - Khoảng cách giữa các dòng (mặc định 12px)
   * @param {number} baselineOffset - Độ lệch font chữ để căn giữa thị giác (mặc định 3.5px)
   */
  ...
  updateData() {
    ...
    // 2. Dữ liệu PV (DC) - Căn Y động theo số String active
    let pvP = 0;
    ...
    // Căn giữa tâm PV Icon (Center Y = 8, step = 14)
    ...
    // 3. PV Hòa Lưới / AC PV - Căn Y động (Tâm Icon Y = 9.5)
    ...
    // 4. Điện Lưới - Căn Y động (Tâm Trụ Điện Y = 20)
    let gridP = 0;
    let rawGridV = 0.0;
    let rawGridF = 0.0;
    ...
    // 5. Tải tiêu thụ - Căn Y động (Tâm Ngôi Nhà Y = 19)
    let loadP = 0;
    if (isThreePhase) {
    ...
    // 6. EPS - Căn Y động (Tâm Icon EPS Y = 19)
    let epsP = 0;
    if (isThreePhase) {
    ...
    // 7. Pin Lưu Trữ - Căn Y động (Tâm Icon Battery Y = 29)
          ...
          <div class="diagram-card">
            ...
            <svg class="diagram-svg" viewBox="0 0 420 245">
              ...
              <g transform="translate(328, 68)">
                <svg x="0" y="0" width="38" height="48" viewBox="0 0 30 40">
                  <path d="M15 2L5 38M15 2L25 38M2 12H28M5 22H25" stroke="#16a34a" stroke-width="3" fill="none" stroke-linecap="round"/>
                </svg>

                <text id="line-grid-1p" x="40" y="0"><tspan id="txt-grid-p" class="svg-txt-bold">0</tspan><tspan class="unit-lbl" dx="3"> W</tspan></text>
                <text id="line-grid-l1" x="40" y="0" style="display:none;"><tspan id="txt-grid-l1" class="svg-txt-bold">0</tspan><tspan class="unit-lbl" dx="3"> W</tspan></text>
                <text id="line-grid-l2" x="40" y="0" style="display:none;"><tspan id="txt-grid-l2" class="svg-txt-bold">0</tspan><tspan class="unit-lbl" dx="3"> W</tspan></text>
                <text id="line-grid-l3" x="40" y="0" style="display:none;"><tspan id="txt-grid-l3" class="svg-txt-bold">0</tspan><tspan class="unit-lbl" dx="3"> W</tspan></text>

                <text id="lbl-grid-mode" x="40" y="0" class="svg-txt-sub" font-weight="bold" fill="#dc2626">Mất lưới</text>
                <text id="line-grid-v" x="40" y="0"><tspan id="txt-grid-v" class="highlight-val">0.0</tspan><tspan class="unit-lbl" dx="3"> V</tspan></text>
                <text id="line-grid-f" x="40" y="0"><tspan id="txt-grid-f" class="highlight-freq">0.00</tspan><tspan class="unit-lbl" dx="3"> Hz</tspan></text>
              </g>

              <!-- Khối 6: EPS -->
              <g id="grp-eps" transform="translate(156, 175)">
                <svg id="icon-eps" x="0" y="0" width="38" height="38" viewBox="0 0 30 30">
              ...
              <g transform="translate(261, 175)">
                <svg id="icon-load" x="0" y="0" width="38" height="38" viewBox="0 0 30 30">
